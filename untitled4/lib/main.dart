  ...
  Widget build(BuildContext context) {
    ...
    var btn = RaisedButton.icon(
      ...
      label: Padding(
        ...
        child: Text('gogo', style: TextStyle(fontSize: 18, color: Colors.white),),
      ),
      color: Colors.lightBlue,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
      onPressed: () {
        animationWrapper.setAlignment();
      },
    );
...
class _AnimationWrapper extends StatefulWidget {
  ...
  setAlignment() {
    _key.currentState?.setAlignment();
  }
}
...
class _AnimationWrapperState extends State<_AnimationWrapper> {
  ...
  Widget build(BuildContext context) {
    var w = Expanded(
      child: AnimatedContainer(
        ...
        child: Icon(Icons.accessibility, color: Colors.lightBlue, size: 50),
  ...
  setAlignment() {
    setState(() {
      widget._alignment = Alignment.centerLeft;
